...
{
    using StateBase = StateMachine<HeishityouControllerStrategy>.StateBase;

    public class HeishityouControllerStrategy : IEnemyControllStrategy
    {
        ...
        public HeishityouControllerStrategy(EnemyController context)
        {
            this.ctx = context;
        }
        private StateMachine<HeishityouControllerStrategy> stateMachine; //ステート遷移管理


        public void OnEnter()
        {
            stateMachine = new StateMachine<HeishityouControllerStrategy>(this);
            stateMachine.Add<StateIdle>((int) StateType.Idle);
            stateMachine.Add<StateResetPosition>((int) StateType.ResetPosition);
        ...
        private class StateIdle : StateBase
        {
            ...
            public override void OnUpdate()
            {
                ...
                Vector3 posDelta = Owner.player.transform.position - ctx.tf.position;
                float distance = posDelta.magnitude;

                // エネミーがプレイヤーを視認するための処理
                // 1. 視界範囲外なら終了
        ...
        private class StateResetPosition : StateBase
        {
            public override void OnStart()
            {
                GameLog.Trace("end ResetPosition");
            }
        ...
        private class StateChase : StateBase
        {
            ...
            public override void OnUpdate()
            {
                ...
                if (Mathf.Abs(posDelta.magnitude) >= ctx.estatus.GetViewRange){
                    ...
                    StateMachine.ChangeState((int) StateType.ResetPosition);
                }
            }
        ...
        private class StateBattle : StateBase
        {
            ...
            public override void OnUpdate()
            {
                ...
                /// 20% - 移動地点を再度指定
                /// <summury>
                if(Mathf.Abs((ctx.tf.position - destination).magnitude) <= 0.2f)
                {
            ...
            private void SetNewDestination()
            {
                ...
                for (int i = 0; i < MAX_ATTEMPTS; i++)
                {
                    ...
                    targetRadius = posDelta.magnitude < 1.5f ? 2.0f : 3.0f;
        ...
        private class StateAttack : StateBase
        {
            ...
            string[] attackPattarn = new string[] {"1", "2", "3", "4", "5"};
            ...
            public override void OnStart()
            {
                ...
                else if(choice < 80)
                {
                    ...
                    ctx.animator.SetState("4", true);
                }
                else
                {
                    ctx.rb.AddForce(ctx.tf.forward * 2.0f, ForceMode.Impulse);
                    currentAnimationNum = 4;
                    ctx.animator.SetState("5", true);
                }
            ...
            public override void OnUpdate()
            {
                ...
                // 確率で連続攻撃を繰り出す
                if(choice <= 50 && 
                    ctx.animator.AnimationFinish(
                    attackPattarn[currentAnimationNum]) >= 0.7f)
                {
                    StateMachine.ChangeState((int) StateType.Attack);
                }
