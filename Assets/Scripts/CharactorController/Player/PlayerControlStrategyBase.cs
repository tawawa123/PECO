...
{
    using StateBase = StateMachine<PlayerControlStrategyBase>.StateBase;

    /// <summary>
    /// プレイヤー操作Strategyの共通基底クラス。
    /// 移動・戦闘・ステルスなど全ステートの実装を保持する。
    /// 変身形態ごとの差分は派生クラスで virtual メソッドをoverrideして表現する。
    /// </summary>
    public abstract class PlayerControlStrategyBase : IPlayerControlStrategy
    {
        private PlayerStatus playerStatus;

        // ctx = PlayerControllerへの参照
        private PlayerController ctx;
        protected PlayerControlStrategyBase(PlayerController context)
        {
            this.ctx = context;
        }
        ...
        protected StateMachine<PlayerControlStrategyBase> stateMachine; //ステート遷移制御
        ...
        /// <summary>
        /// 変身形態ごとに固有の入力を処理するフック。
        /// 既定（通常状態）では何もしない。派生クラスでoverrideして拡張する。
        /// </summary>
        protected virtual void HandleTransformInput() { }
        ...
        private class StateBackstab : StateBase
        {
            ...
            public override void OnStart()
            {
                ...
                {
                    StateMachine.ChangeState((int) StateType.StealthAttack);
                    return; // ステルスアタックへ遷移したらBackstabアニメは再生しない
                }
